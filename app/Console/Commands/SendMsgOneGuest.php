...
{
	protected $signature = 'send:send_msg_one_guest';
	protected $description = 'Comando para efetuar o registro e disparo do primeiro envio de notificacao do hospede.';
	...
	public function handle()
	{
		...
		$guests = Guest::select(
						...
						->where('guests.msg_1_status_send', '!=' ,'1')
						->where('hospitalities.ind_msg_1', '!=' ,'0')
						->whereRaw('
								CASE
									WHEN hospitalities.ind_msg_1 = 1 THEN DATE_SUB(guests.date_checkin, INTERVAL 1 DAY) = DATE(NOW())
									WHEN hospitalities.ind_msg_1 = 2 THEN DATE_SUB(guests.date_checkin, INTERVAL 2 DAY) = DATE(NOW())
									WHEN hospitalities.ind_msg_1 = 3 THEN DATE_SUB(guests.date_checkin, INTERVAL 3 DAY) = DATE(NOW())
									WHEN hospitalities.ind_msg_1 = 4 THEN DATE_SUB(guests.date_checkin, INTERVAL 4 DAY) = DATE(NOW())
									WHEN hospitalities.ind_msg_1 = 5 THEN DATE_SUB(guests.date_checkin, INTERVAL 5 DAY) = DATE(NOW())
									WHEN hospitalities.ind_msg_1 = 6 THEN DATE_SUB(guests.date_checkin, INTERVAL 6 DAY) = DATE(NOW())
									WHEN hospitalities.ind_msg_1 = 7 THEN DATE_SUB(guests.date_checkin, INTERVAL 7 DAY) = DATE(NOW())
									WHEN hospitalities.ind_msg_1 = 8 THEN DATE_SUB(guests.date_checkin, INTERVAL 15 DAY) = DATE(NOW())
									WHEN hospitalities.ind_msg_1 = 9 THEN DATE_SUB(guests.date_checkin, INTERVAL 1 MONTH) = DATE(NOW())
									ELSE FALSE
								END
						')
						->get();
		...
		foreach ($guests as $key => $item)
		{
			...
			$obj_data = (object) array(
				...
				"template_id"             => $item->msg_1_template_id,
				"guest_id"                => $item->id,
