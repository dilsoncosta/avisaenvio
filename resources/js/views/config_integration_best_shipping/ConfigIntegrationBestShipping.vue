				...
				<div class="header">
					<div>
						<ol class="page-content-title">
							...
							<li class="active">Integração Melhor Envio</li>
						</ol>
					</div>
					<div></div>
				</div>
							...
							<td>
								<table>
									<tbody>
										<tr>
											<td>
												<textarea rows="7" cols="50" v-model="token" :disabled="situation == 1"></textarea>
											</td>
										</tr>
									</tbody>
								</table>
...
onMounted( async () => {
	...
	try {
		const response = await store.dispatch('getStatusIntegrationBestShippingConfigIntegrationBestShipping');
...
const activeIntegrationWhatsApp = async () => {
	...
	{
		return show_msgbox('O Campo TOKEN é obrigatório!', 'warning');
	}
	...
	try
	{
		const response = await store.dispatch('activeConfigIntegrationBestShipping', {
			token: token.value,
			situation: 1,
			uuid: uuid.value
		});
...
const disabledIntegrationWhatsApp = async () => {
	...
	try
	{
		await store.dispatch('disabledConfigIntegrationBestShipping', {
			token: token.value,
			situation: 0,
			uuid: uuid.value
		});
