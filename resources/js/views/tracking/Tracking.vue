		...
		<div class="page-content">
			<div class="table">
				<div class="header">
					...
					<div>
						<a href="#" class="f_show_delete" @click="destroyTracking" v-if="$can('delete_tracking')"><i class="fas fa-lg fa-trash-alt"></i></a>
					...
					<table>
						...
						<tbody>
							...
							<tr v-else v-for="(item, index) in items.data" :key="index">
								...
								<td>
									<div class="options">
										<a v-if="$can('view_tracking')"><i class="fas fa-file-alt fa-lg fa-fw" @click.prevent="btnRecordTracking(item)"></i></a>
								...
								<td v-if="item.situation == 1" class="td_format"><b class="pendent">Pendente</b></td>
								<td v-else-if="item.situation == 2" class="td_format"><b class="in_progress">Em Andamento</b></td>
...
/* Events */
watch(() => store.state.tracking.data,
			() => {
				items.value = store.state.tracking.data;
});
...
const btnSearch = async () => {
	...
	try {
		await store.dispatch('getTracking', {
...
const btnRecordTracking = async (item) => {
	formRecord.value = item;
	statusModalRecord.value = true;
	titleModal.value = 'Visualizar Rastreio';
}
...
const destroyTracking = async () => {
	...
	if(await confirm_alert('Tem certeza que deseja apagar esse registro. Confirma?') == true)
	{
		...
		store.dispatch('destroyTracking', {
			ids : ids
		})
		.then((response) => {
			refreshTableOfDatas();
